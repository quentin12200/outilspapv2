...
async def get_entreprise_data(siren: str, db: Session = Depends(get_session)):
    """
    Récupère toutes les données d'une entreprise par son SIREN:
    ...
    if not normalized_siren or len(normalized_siren) != 9:
        raise HTTPException(
            status_code=400,
            detail=f"SIREN invalide. Attendu: 9 chiffres. Reçu: {len(normalized_siren)} chiffres"
        )
    
    # 1. Récupérer les informations de l'entreprise via Sirene
    entreprise = {}
    try:
        ...
        logger.warning(f"Impossible de récupérer les infos Sirene pour SIREN {normalized_siren}: {e}")
        entreprise = {"siren": normalized_siren, "nom": ""}
    
    # 2. Récupérer tous les PV d'élections pour ce SIREN
    # Les SIRET commencent par le SIREN, donc on filtre avec LIKE
    pv_events = (
        db.query(PVEvent)
        .filter(PVEvent.siret.like(f"{normalized_siren}%"))
    ...
    for pv in pv_events:
        pv_list.append({
            ...
            "cycle": pv.cycle,
            "c4_inscrits": pv.c4_inscrits,
            "c4_cgt": pv.c4_cgt,
            "c4_cfdt": pv.c4_cfdt,
            "c4_fo": pv.c4_fo,
            "c4_cfe_cgc": pv.c4_cfe_cgc,
            "c4_unsa": pv.c4_unsa,
            "c4_sud": pv.c4_sud,
            "c4_fsu": pv.c4_fsu,
            "c4_cftc": pv.c4_cftc,
    ...
    # 3. Récupérer toutes les invitations pour ce SIREN
    invitations = (
        db.query(Invitation)
        .filter(Invitation.siret.like(f"{normalized_siren}%"))
    ...
    # 4. Récupérer tous les établissements via Pappers
    etablissements = []
    try:
        result = await pappers_api.get_etablissements_by_siren(normalized_siren)
        if result.get("success"):
            etablissements = result.get("etablissements", [])
    except Exception as e:
        logger.error(f"Erreur lors de la récupération des établissements pour SIREN {normalized_siren}: {e}")
