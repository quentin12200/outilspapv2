...
async def get_entreprise_fiche_complete(
    ...
    try:
        ...
        # ==================== 7. CALCUL SCORE POTENTIEL CGT ====================
        score_potentiel = _calculer_score_potentiel_cgt(
        ...
        # ==================== RÉPONSE FINALE ====================
        logger.info(f"✅ Fiche entreprise récupérée : {stats['nb_pv_total']} PV totaux, {stats['nb_invitations_pap']} invitations PAP, {stats['nb_etablissements']} établissements, Score potentiel CGT: {score_potentiel['score']}/100")
...
def _calculer_score_potentiel_cgt(pv_par_cycle, totaux_par_cycle, info_base, pappers_data, stats):
    """
    Calcule un score de potentiel CGT pour une entreprise (/100).

    Critères:
    - Présence CGT actuelle (30 points)
    - Évolution C3 → C4 (25 points)
    - Taille entreprise / effectifs (20 points)
    - Secteur d'activité (15 points)
    - Concurrence syndicale (10 points)

    Returns:
        dict avec score, niveau, détails
    """
    score = 0
    details = {}

    # ==================== 1. PRÉSENCE CGT ACTUELLE (30 points) ====================
    score_presence = 0

    # Score C4
    total_pv_c4 = totaux_par_cycle.get("C4", {}).get("nb_pv", 0) or totaux_par_cycle.get("Cycle 4", {}).get("nb_pv", 0)
    presence_cgt_c4 = totaux_par_cycle.get("C4", {}).get("presence_cgt", 0) or totaux_par_cycle.get("Cycle 4", {}).get("presence_cgt", 0)

    if total_pv_c4 > 0:
        pourcentage_presence = (presence_cgt_c4 / total_pv_c4) * 100

        if pourcentage_presence >= 75:
            score_presence = 30  # Très forte présence
        elif pourcentage_presence >= 50:
            score_presence = 22  # Bonne présence
        elif pourcentage_presence >= 25:
            score_presence = 15  # Présence moyenne
        elif pourcentage_presence > 0:
            score_presence = 8   # Faible présence
        else:
            score_presence = 0   # Pas de CGT

        details["presence_cgt"] = {
            "score": score_presence,
            "pourcentage": round(pourcentage_presence, 1),
            "etablissements_cgt": presence_cgt_c4,
            "total_etablissements": total_pv_c4
        }
    else:
        score_presence = 5  # Bonus si pas encore de PV (terrain vierge)
        details["presence_cgt"] = {
            "score": score_presence,
            "pourcentage": 0,
            "etablissements_cgt": 0,
            "total_etablissements": 0,
            "raison": "Pas de PV C4 - Terrain vierge"
        }

    score += score_presence

    # ==================== 2. ÉVOLUTION C3 → C4 (25 points) ====================
    score_evolution = 0

    total_pv_c3 = totaux_par_cycle.get("C3", {}).get("nb_pv", 0) or totaux_par_cycle.get("Cycle 3", {}).get("nb_pv", 0)
    presence_cgt_c3 = totaux_par_cycle.get("C3", {}).get("presence_cgt", 0) or totaux_par_cycle.get("Cycle 3", {}).get("presence_cgt", 0)

    if total_pv_c3 > 0 and total_pv_c4 > 0:
        pourcentage_c3 = (presence_cgt_c3 / total_pv_c3) * 100
        pourcentage_c4 = (presence_cgt_c4 / total_pv_c4) * 100
        evolution = pourcentage_c4 - pourcentage_c3

        if evolution >= 20:
            score_evolution = 25  # Très forte progression
        elif evolution >= 10:
            score_evolution = 18  # Bonne progression
        elif evolution >= 0:
            score_evolution = 12  # Stabilité
        elif evolution >= -10:
            score_evolution = 6   # Léger recul
        else:
            score_evolution = 0   # Fort recul

        details["evolution"] = {
            "score": score_evolution,
            "evolution": round(evolution, 1),
            "c3": round(pourcentage_c3, 1),
            "c4": round(pourcentage_c4, 1)
        }
    elif total_pv_c4 > 0 and total_pv_c3 == 0:
        # Nouvelle implantation
        score_evolution = 20
        details["evolution"] = {
            "score": score_evolution,
            "raison": "Nouvelle implantation CGT (pas de C3)"
        }
    else:
        score_evolution = 10  # Neutre si pas de données
        details["evolution"] = {
            "score": score_evolution,
            "raison": "Pas assez de données pour calculer l'évolution"
        }

    score += score_evolution

    # ==================== 3. TAILLE ENTREPRISE / EFFECTIFS (20 points) ====================
    score_taille = 0

    effectif = info_base.get("effectif_siren", 0) or (pappers_data.get("effectif") if pappers_data else 0) or 0
    ...
    if effectif >= 500:
        score_taille = 20  # Grande entreprise - fort potentiel
    elif effectif >= 200:
        score_taille = 16  # Moyenne/grande entreprise
    elif effectif >= 50:
        score_taille = 12  # PME importante
    elif effectif >= 20:
        score_taille = 8   # Petite entreprise
    elif effectif > 0:
        score_taille = 4   # TPE
    else:
        score_taille = 10  # Effectif inconnu - potentiel moyen

    details["taille"] = {
        "score": score_taille,
        "effectif": effectif,
        "nb_etablissements": stats.get("nb_etablissements", 0)
    }

    score += score_taille

    # ==================== 4. SECTEUR D'ACTIVITÉ (15 points) ====================
    score_secteur = 0

    # Secteurs à fort potentiel CGT (historiquement)
    secteurs_forts = ["industrie", "métallurgie", "chimie", "transport", "énergie", "construction", "santé"]
    secteurs_moyens = ["commerce", "service", "alimentation", "logistique"]

    activite = (pappers_data.get("libelle_activite", "") if pappers_data else "") or info_base.get("libelle_activite", "")
    activite_lower = activite.lower() if activite else ""

    if any(secteur in activite_lower for secteur in secteurs_forts):
        score_secteur = 15  # Secteur historiquement pro-CGT
    elif any(secteur in activite_lower for secteur in secteurs_moyens):
        score_secteur = 10  # Secteur moyen
    else:
        score_secteur = 7   # Secteur inconnu/neutre

    details["secteur"] = {
        "score": score_secteur,
        "activite": activite or "Non renseigné"
    }

    score += score_secteur

    # ==================== 5. CONCURRENCE SYNDICALE (10 points) ====================
    score_concurrence = 0

    # Compter le nombre d'organisations présentes en C4
    nb_organisations = 0
    pv_c4 = pv_par_cycle.get("C4", []) or pv_par_cycle.get("Cycle 4", [])

    if pv_c4:
        # Prendre le PV avec le plus d'organisations
        for pv in pv_c4:
            nb_org_pv = len(pv.get("organisations", []))
            if nb_org_pv > nb_organisations:
                nb_organisations = nb_org_pv

    if nb_organisations == 0:
        score_concurrence = 10  # Pas de syndicats = terrain libre
    elif nb_organisations == 1:
        score_concurrence = 8   # CGT seule ou un seul concurrent
    elif nb_organisations == 2:
        score_concurrence = 6   # 2 organisations (faible concurrence)
    elif nb_organisations <= 4:
        score_concurrence = 4   # Concurrence modérée
    else:
        score_concurrence = 2   # Forte concurrence

    details["concurrence"] = {
        "score": score_concurrence,
        "nb_organisations": nb_organisations
    }
    ...
    if score >= 80:
        niveau = "Haute opportunité"
        couleur = "green"
        emoji = "🎯"
        description = "Entreprise à fort potentiel de renforcement CGT. Priorité maximale !"
    elif score >= 60:
        niveau = "Opportunité intéressante"
        couleur = "blue"
        emoji = "📈"
        description = "Bon potentiel de développement. Action recommandée."
    elif score >= 40:
        niveau = "Potentiel moyen"
        couleur = "yellow"
        emoji = "⚖️"
        description = "Potentiel modéré. Évaluer les ressources disponibles."
    elif score >= 20:
        niveau = "Potentiel faible"
        couleur = "orange"
        emoji = "⚠️"
        description = "Faible potentiel. Prioriser d'autres entreprises."
    else:
        niveau = "CGT déjà fortement implantée"
        couleur = "purple"
        emoji = "💪"
        description = "CGT déjà bien présente. Focus sur le maintien."
